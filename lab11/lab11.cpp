...
Result computeExplicit(long double h)
{
    ...
    if (lambda > 0.5L) {
        cerr << "Error: lambda exceeds stability limit (lambda = " << lambda << ")" << endl;
        exit(1);
    }
...
Result computeLaasonen(long double h)
{
    Result result;
    long double lambda = D * dt / (h * h);
    int nx = static_cast<int>((2.0L * a) / h) + 1;
...
int main()
{
    long double h_explicit = sqrt(10) / 200.0L;
    long double h_laasonen = 0.01L;
    int nt = static_cast<int>(tmax / dt) + 1;

    Result explicitResult = computeExplicit(h_explicit);
    Result laasonenResult = computeLaasonen(h_laasonen);
    ...
    for (int i = 0; i < laasonenResult.x.size(); ++i)
    {
        fp1 << laasonenResult.x[i] << " "
            << laasonenResult.numerical[i][nt - 1] << " "
            << laasonenResult.analytical[i][nt - 1] << "\n";
    }
    fp1.close();
